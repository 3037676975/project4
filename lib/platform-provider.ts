...
export async function ensurePlatformProviderConfigs(admin: PlatformContext) {
  ...
  if (!source?.tenant_id) return { adopted: 0, tenantId: null as string | null };
  ...
  if (adopted) {
    await writePlatformAudit(admin, "provider_configs.adopted", "platform_provider", null, {
      sourceTenantId: source.tenant_id, adoptedKinds: adopted,
    });
  }
  return { adopted, tenantId: source.tenant_id };
}
