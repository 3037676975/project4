  ...
  return (
    <div style={{ position: "fixed", right: 18, bottom: 18, zIndex: 1000, width: "min(410px, calc(100vw - 28px))", height: "min(650px, calc(100vh - 36px))", maxHeight: "calc(100vh - 36px)", display: "flex", flexDirection: "column", overflow: "hidden", borderRadius: 28, border: "1px solid rgba(148,163,184,.25)", background: "rgba(255,255,255,.98)", boxShadow: "0 28px 90px rgba(15,23,42,.22), 0 8px 24px rgba(79,70,229,.08)", fontFamily: "var(--font-geist-sans), system-ui, sans-serif", backdropFilter: "blur(22px)" }}>
      <div style={{ position: "relative", padding: "18px 18px 14px", color: "white", background: "linear-gradient(135deg,#0f172a 0%,#312e81 55%,#6d28d9 100%)" }}>
        ...
        <div style={{ position: "relative", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6, marginTop: 15, padding: 4, borderRadius: 14, background: "rgba(255,255,255,.09)" }}>
      ...
      <div ref={messageBoxRef} style={{ flex: 1, minHeight: 0, overflowY: "auto", padding: "18px 16px 14px", background: "linear-gradient(180deg,#f8fafc 0%,#ffffff 45%)" }}>
        {messages.map((message) => <div key={message.id} style={{ display: "flex", justifyContent: message.role === "user" ? "flex-end" : "flex-start", marginBottom: 14 }}><div style={{ maxWidth: "86%" }}>{message.role === "ai" && <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 6, color: "#64748b", fontSize: 11 }}><span style={{ width: 20, height: 20, borderRadius: 7, display: "grid", placeItems: "center", background: "#eef2ff", color: "#4f46e5", fontWeight: 800 }}>K</span> KnowFlow AI</div>}<div style={{ padding: "11px 13px", borderRadius: message.role === "user" ? "17px 17px 5px 17px" : "17px 17px 17px 5px", background: message.role === "user" ? "linear-gradient(135deg,#4f46e5,#6d28d9)" : message.error ? "#fff7ed" : "#fff", color: message.role === "user" ? "white" : "#1e293b", border: message.role === "user" ? "none" : `1px solid ${message.error ? "#fed7aa" : "#e2e8f0"}`, boxShadow: message.role === "user" ? "0 8px 18px rgba(79,70,229,.2)" : "0 6px 18px rgba(15,23,42,.05)", fontSize: 13.5, lineHeight: 1.65, whiteSpace: "pre-wrap" }}>{message.text}</div>{message.meta && <div style={{ marginTop: 5, color: "#94a3b8", fontSize: 10.5, textAlign: message.role === "user" ? "right" : "left" }}>{message.meta}</div>}</div></div>)}
        {loading && <div style={{ display: "flex", alignItems: "center", gap: 8, color: "#64748b", fontSize: 12 }}><span style={{ width: 28, height: 28, borderRadius: 10, display: "grid", placeItems: "center", background: "#eef2ff", color: "#4f46e5" }}>✦</span><span>正在查询知识库… 最多等待 15 秒</span></div>}
      </div>
