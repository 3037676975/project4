import { accountAccess, assertSameOrigin, attachAccountByEmail, authenticateUser, authRouteError, createUserAccount, getActiveAccountByEmail, issueSession, normalizeEmail, portalDestination, recordSuccessfulLogin, validLoginPortal } from "../../../../lib/app-auth";
...
export async function POST(request: Request) {
  try {
    ...
    if (mode === "password" && !account && runtime.APP_ENV === "local" && verifyLocalCredentials(email, password)) {
      ...
      if (!existing) {
        account = await createUserAccount({ email: normalized, displayName: runtime.LOCAL_AUTH_NAME || "本地超级管理员", password, mustChangePassword: true });
