...
# Use kernel-backed flock instead of the legacy mkdir lock. If two webhook
# deliveries arrive together, the second waits for the first and then exits if
# that SHA was already deployed. A killed process cannot leave a permanent stale
# flock behind. The different filename intentionally avoids any old .lock folder.
exec 9>"$LOCK_FILE"
