...
export default function PlatformProviderSettings() {
  ...
  useEffect(() => {
    let active = true;
    const refresh = () => void api<ServiceHealth>("/api/platform/services").then((result) => { if (active) setServiceHealth(result); }).catch(() => { if (active) setServiceHealth(null); });
  ...
  return <div className="platform-provider-page">{notice && <div className={`toast ${notice.kind}`}><span>{notice.kind === "ok" ? "✓" : "!"}</span>{notice.text}<button onClick={() => setNotice(null)}>×</button></div>}<section className="service-intro card"><div><p className="section-kicker">Platform provider control</p><h2>全平台模型与文档服务</h2><p>这里的配置由超级管理员统一维护，企业账号只能使用，不能查看或修改密钥。</p></div><div className="service-status"><span className={generation.configured ? "ready" : ""}>DeepSeek</span><span className={embedding.configured ? "ready" : ""}>Embedding</span><span className={rerank.configured ? "ready" : ""}>Rerank</span><span className={ocr.configured ? "ready" : ""}>OCR · {ocr.provider}</span></div></section>{serviceHealth && <section className="runtime-service-grid">{serviceHealth.services.map((service) => <article key={service.id} className={`runtime-service ${service.status}`}><i/><div><b>{service.name}</b><span>{service.detail}</span></div><em>{service.status === "healthy" ? "运行中" : service.status === "degraded" ? "加载中" : "未启动"}</em></article>)}</section>}<section className="card platform-provider-boundary"><div><b>统一生效</b><span>保存后，全部租户的质量测试、官网客服和客户 API 使用同一套平台服务。</span></div><div><b>旧密钥已接管</b><span>首次进入会自动复制原有加密配置，不要求重新填写；原租户配置保留用于回滚。</span></div><div><b>企业不可见</b><span>企业后台不显示配置表单，服务端也拒绝企业账号写入或测试密钥。</span></div></section><div className="provider-grid">{providerForm(generation, "generation")}{providerForm(embedding, "embedding")}{providerForm(rerank, "rerank")}{providerForm(ocr, "ocr")}</div></div>;
}
