  ...
  return <main style={{ minHeight: "100vh", background: "#f5f7fb", color: "#172033", padding: "32px" }}>
    <div style={{ maxWidth: 1400, margin: "0 auto" }}>
      <header style={{ display: "flex", gap: 18, alignItems: "center", justifyContent: "space-between", marginBottom: 24 }}>
        <div><p style={{ margin: 0, color: "#6d5dfc", fontWeight: 800, letterSpacing: ".12em", fontSize: 12 }}>PAYMENT LAB</p><h1 style={{ margin: "6px 0", fontSize: 34 }}>支付订单与回调监控</h1><p style={{ margin: 0, color: "#687386" }}>参考 Project3 Payment Lab：支付宝 RSA2 + 微信支付 V2 Native，全站共用统一订单、回调验签和支付日志。</p></div>
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}><Link href="/platform/wechat-v2" style={{ ...buttonStyle, background: "#eaf8f2", color: "#117354", borderColor: "#bfe7d7" }}>微信 V2 全局配置</Link><Link href="/platform" style={buttonStyle}>返回平台后台</Link><button type="button" onClick={() => void load()} disabled={busy} style={{ ...buttonStyle, border: 0, background: "#5f45e8", color: "white", cursor: "pointer" }}>{busy ? "刷新中…" : "刷新数据"}</button></div>
      </header>
      ...
      <section style={{ display: "grid", gridTemplateColumns: "repeat(4,minmax(0,1fr))", gap: 14, marginBottom: 18 }}>
        ...
        <Card label="支付实现" value="RSA2 + WeChat V2" detail="Native / unifiedorder / orderquery" />
      </section>
