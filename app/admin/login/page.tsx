...
export default async function AdminLoginPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  if (account) { const access = await accountAccess(account); const destination = portalDestination(access, "admin") || (access.destination !== "/login" ? access.destination : null); if (destination) redirect(destination); }
  return <LoginClient portal="admin" loggedOut={query.logged_out === "1"} errorCode={typeof query.error === "string" ? query.error : ""}/>;
}
