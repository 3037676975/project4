...
export async function GET(request: Request) {
  ...
  return javascript(`(()=>{if(document.getElementById('knowflow-widget-root'))return;const root=document.createElement('div');root.id='knowflow-widget-root';root.style.cssText='position:fixed;right:22px;bottom:22px;z-index:2147483000;font-family:system-ui,sans-serif';const frame=document.createElement('iframe');frame.src=${JSON.stringify(frameUrl)};frame.title=${JSON.stringify(assistant.brandName)};frame.allow='clipboard-write';frame.style.cssText='display:none;width:min(420px,calc(100vw - 24px));height:min(680px,calc(100vh - 92px));border:0;border-radius:18px;box-shadow:0 18px 60px rgba(15,23,42,.25);background:#fff;margin-bottom:12px';const button=document.createElement('button');button.type='button';button.setAttribute('aria-label','打开智能客服');button.innerHTML='✦';button.style.cssText=${JSON.stringify(`float:right;width:58px;height:58px;border:0;border-radius:50%;background:${color};color:#fff;font-size:24px;cursor:pointer;box-shadow:0 12px 30px ${color}66;transition:transform .2s ease`)};let open=false;button.onclick=()=>{open=!open;frame.style.display=open?'block':'none';button.innerHTML=open?'×':'✦';button.style.transform=open?'rotate(90deg)':'none'};root.append(frame,button);document.body.append(root)})();`);
}
