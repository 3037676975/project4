...
import CustomerServiceCenter from "./customer-service-center";
...
type NavLabel = "概览" | "知识库" | "助手" | "质量测试" | "客服中心" | "成员与权限" | "套餐与账单" | "API 密钥" | "用量与成本" | "渠道与合规";
const navGroups: Array<{ group: string; items: Array<[NavLabel, string]> }> = [
  { group: "日常工作", items: [["概览", "01"], ["知识库", "02"], ["助手", "03"], ["质量测试", "04"], ["客服中心", "05"]] },
...
export default function Dashboard({ user, isPlatformAdmin, initialTenantId, logoutHref }: { user: { displayName: string; email: string }; isPlatformAdmin: boolean; initialTenantId?: string; logoutHref: string }) {
  ...
  async function finishOnboarding(result: { demo: { indexStatus: string }; widget: { published: boolean }; warning?: string | null }) {
    setShowOnboarding(false); await loadAll(); setActive("客服中心");
  ...
  function overview() {
    return <>
      <section className="hero-strip"><div><p className="section-kicker">Commercial AI employee</p><h2>从“能回答”升级为“能获客、能转人工、能证明产出”</h2><p>多租户知识库与 RAG 继续作为底座；新增官网客服、销售线索、人工工单、知识缺口和成交金额，让系统具备真正可销售的业务闭环。</p></div><button className="primary-button fit" onClick={() => setActive("客服中心")}>发布官网客服 <span>→</span></button></section>
  ...
  return <><main className="app-shell enterprise-shell">
    ...
    <section className="workspace">
      ...
      <div className="page-content">{active === "概览" && overview()}{active === "知识库" && knowledge()}{active === "助手" && assistantView()}{active === "质量测试" && quality()}{active === "API 密钥" && apiKeysView()}{active === "客服中心" && <CustomerServiceCenter canAdmin={Boolean(canAdmin)} members={tenant?.members || []} onNotice={commercialNotice}/>} {active === "成员与权限" && membersView()}{active === "套餐与账单" && billingView()}{active === "用量与成本" && usageView()}{active === "渠道与合规" && <OperationsPanel assistantId={assistant.id} canAdmin={Boolean(canAdmin)} onNotice={commercialNotice}/>}</div>
    </section>
