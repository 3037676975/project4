...
export default function Dashboard({ user, isPlatformAdmin, logoutHref }: { user: { displayName: string; email: string }; isPlatformAdmin: boolean; logoutHref: string }) {
  ...
  function billingView() {
    return <><section className="tenant-hero card"><div><p className="section-kicker">Enterprise workspace</p><h2>{tenant?.tenant.name || "工作区"}</h2><p>{tenant?.tenant.slug} · 当前角色 {tenant?.currentUser.role} · {knowledgeBases.length} 个隔离知识库</p><button className="secondary-button fit" onClick={() => void completeOnboarding()}>{tenant?.tenant.onboardingCompleted ? "修改企业资料" : "完成企业注册"}</button></div><div className="credit-orb"><span>Credits</span><strong>{count(billing?.usage.creditsBalance || 0)}</strong></div></section>
      ...
      {billing?.payment.ready && <section className="card checkout-channel"><div><p className="section-kicker">Checkout channel</p><h2>选择付款方式</h2><span>仅显示平台超级管理员已经启用并通过校验的渠道。</span></div><div>{billing.payment.channels.filter((item) => item.ready).map((channel) => <label className={paymentProvider === channel.provider ? "active" : ""} key={channel.provider}><input type="radio" name="payment-provider" checked={paymentProvider === channel.provider} onChange={() => setPaymentProvider(channel.provider)}/><i>{channel.provider === "wechat" ? "微" : channel.provider === "alipay" ? "支" : "聚"}</i><span><b>{channel.name}</b><small>{channel.feeRateBps ? `费率 ${(channel.feeRateBps / 100).toFixed(2)}%` : "平台不加收渠道费"}</small></span></label>)}</div></section>}
