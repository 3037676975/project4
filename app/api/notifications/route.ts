...
import { supportEmailNotificationsAllowed } from "../../../lib/platform-settings";
...
export async function GET(request: Request) {
  try {
    ...
    const [configs, outbox, emailAllowedByPlatform] = await Promise.all([
      listNotificationConfigs(context.tenantId),
      DB.prepare(`SELECT id, channel, event_type, entity_type, entity_id, status, attempts, last_error, created_at, sent_at
    ...
    return Response.json({ emailAllowedByPlatform, configs, outbox: (outbox.results as Array<Record<string, unknown>>).map((row) => ({ id: row.id, channel: row.channel, eventType: row.event_type, entityType: row.entity_type, entityId: row.entity_id, status: row.status, attempts: row.attempts, lastError: row.last_error, createdAt: row.created_at, sentAt: row.sent_at })) });
...
export async function POST(request: Request) {
  try {
    ...
    if (channel === "email" && !(await supportEmailNotificationsAllowed())) return Response.json({ error: "超级管理员尚未允许企业启用客服邮件通知。" }, { status: 403 });
